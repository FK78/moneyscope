...
export async function saveTrueLayerConnection(
  ...
  const [connection] = await db.insert(truelayerConnectionsTable).values({
    user_id: userId,
...
async function getValidToken(connectionId: number): Promise<string> {
  ...
  await db.update(truelayerConnectionsTable).set({
    access_token: tokens.access_token,
    refresh_token: tokens.refresh_token,
...
function mapAccountType(tlType: string): 'currentAccount' | 'savings' | 'creditCard' | 'investment' {
...
export async function importFromTrueLayer() {
  ...
  for (const connection of connections) {
    ...
    for (const tlAccount of tlAccounts) {
      ...
      let balance = 0;
      try {
        const balanceData = await fetchBalance(accessToken, tlAccount.account_id);
      ...
      if (existing) {
        // Update balance
        await db.update(accountsTable).set({ balance })
        ...
        const [created] = await db.insert(accountsTable).values({
          user_id: userId,
          name: tlAccount.display_name || `${tlAccount.provider?.display_name ?? 'Bank'} Account`,
      ...
      let tlTransactions;
      try {
        tlTransactions = await fetchTransactions(accessToken, tlAccount.account_id, from, to);
      } catch {
      ...
      const uncategorised = categories.find(c => c.name.toLowerCase() === 'uncategorised')
        ?? categories.find(c => c.name.toLowerCase() === 'other')
        ?? categories[0];

      for (const tlTxn of tlTransactions) {
        // Dedup by truelayer_id
        const [existingTxn] = await db.select({ id: transactionsTable.id })
          .from(transactionsTable)
          .where(eq(transactionsTable.truelayer_id, tlTxn.transaction_id));

        if (existingTxn) continue;

        const isExpense = tlTxn.transaction_type === 'DEBIT' || tlTxn.amount < 0;
...
export async function disconnectTrueLayer(connectionId: number) {
  ...
  await db.delete(truelayerConnectionsTable)
    .where(and(
